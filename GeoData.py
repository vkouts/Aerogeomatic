...
class Road:
    ...
    """Основной объект для работы с геоданными.
      ...
      При старте, если не установлен флаг (не существует файла FirstRun.flag),
      вычитывает исходные данные из файлов в директории ./in/
      Основные свойства:
      ...
      Основные методы:
        ...
        RoadAZS(Rcode, Begin_km, End_km, Date) - выводит список АЗС
        на заданном участке
           Rcode - код дороги
           Begin_km - начальный участок
           End_km - конечный участок
           Date - актуальная дата
        RoadExits(AZSs, Begin_km, End_km, Date) - выводит список съездов АЗС
        на заданном участке
        ...
        for AZS in AZSs:
            ...
            while row is not None:
                ...
                if u'АЗС' in row[3] and (Date_ != '' and (Date_ > data_add_ and Date_ < data_delete_)):
                    EXITS.append(dict(
                      code=row[0],
                      position=km_to_kmm(row[1]),
                      transverse=row[2],
                      name=row[3],
                      material=row[4],
                      tech_condition=row[5],
                      date_add=row[6],
                      date_del=row[7]))
                row = self.cur.fetchone()
        return EXITS
